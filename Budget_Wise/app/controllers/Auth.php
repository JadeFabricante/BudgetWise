...
class Auth extends Controller {
    ...
    public function register() {
        if ($this->form_validation->submitted()) {
            ...
            $email = $this->io->post('email');
            $gender = $this->io->post('gender'); // Capture the gender input
            $email_token = bin2hex(random_bytes(50));
    
            // Add validation rules, including for gender
            $this->form_validation
                ...
                ->name('gender') // Validate gender
                    ->required('Please select a gender.')
                    ->in_list(['male', 'female', 'other'], 'Invalid gender selection.')
                ->name('password')
                    ->required()
            ...
            if ($this->form_validation->run()) {
                if ($this->lauth->register($username, $email, $this->io->post('password'), $email_token, $gender)) {
